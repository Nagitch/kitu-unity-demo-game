...
before running the commands below. Always use `python3 tools/run.py` for
commands that consume the selected Kitu configuration: default setup selects
the original demo checkout, while `tools/setup.py --kitu-path` selects an
isolated effective copy.

`app` is a small application built on top of the Kitu framework crates.
It exists for vertical-slice development, Web Admin hosting, and CI scenario tests.

The [delivery matrix](../docs/verification/arena-delivery/README.md) links all
Arena stages, merged PRs and comparison evidence. Use the shared
[build and verification recipe](../docs/specs/arena-build-verification.md) for
...
- `../tests/scenarios/`: checked-in scenario fixtures for CI.
- `tests/`: scenario test harnesses that execute fixtures against the Kitu runtime.
- root `docker-compose.yml` via `python3 tools/compose.py`: standalone app
  host, Web Admin frontend and optional WebTransport gateway.
...
Then open the Web Admin frontend separately, or use the root Compose wrapper:
...
[boss contract, diagnostics and native options](../docs/specs/arena-boss-scripts.md).
...
[Arena contract](../docs/specs/arena-runtime-contract.md) and
[full-game evidence](../docs/verification/arena-progression/results.json).
...
[inspection contract](../docs/specs/arena-inspection.md) for exact integer
...
The host reads `app/content/arena.tmd` relative to its working directory;
...
`app/.arena/runs/<runtime-session>/<run>.json`; use
...
```sh
python3 tools/run.py cargo run --locked -p kitu-demo-game --bin arena-content -- validate app/content/arena.tmd
python3 tools/run.py cargo test --locked -p kitu-demo-game -p kitu-data-tmd -p kitu-runtime
python3 tools/run.py pnpm --dir admin check
python3 tools/run.py pnpm --dir admin lint
python3 tools/run.py pnpm --dir admin build
```

`arena-content create-reference <path>` generates a new reference-valued TMD for
development. The separate [live CLI](https://github.com/Nagitch/kitu-logic-processor/blob/develop/doc/specs/live-shell.md) operates the running host.
The macOS PlayMode
...
Admin. See [stage 6 evidence](../docs/verification/arena-tanu/results.json).
...
```sh
mkdir -p .tmp/content
python3 tools/run.py cargo run --locked -p kitu-demo-game --bin arena-content -- create-sqlite .tmp/content/default.sqlite
python3 tools/run.py cargo run --locked -p kitu-demo-game --bin arena-content -- create-plan .tmp/content/sample.arena.json
python3 tools/run.py cargo run --locked -p kitu-demo-game --bin arena-content -- validate .tmp/content/sample.arena.json
python3 tools/run.py env KITU_ARENA_CONTENT=.tmp/content/sample.arena.json cargo run --locked -p kitu-demo-game --bin kitu-demo-game-admin-host
```
...
matching archived runtime. See the [source contract](../docs/specs/arena-content-sources.md)
...
See the [full file/version/endpoint contract](../docs/specs/arena-replay.md).
...
Admin/Unity state equality, stop/reconnect and paused live restoration. Stage 9 adds the [live CLI and browser Shell](https://github.com/Nagitch/kitu-logic-processor/blob/develop/doc/specs/live-shell.md).
...
The [native ABI contract](../docs/specs/arena-native-abi.md) includes macOS build
...
Generate editable copies with `python3 tools/run.py cargo run --locked -p
kitu-demo-game --bin arena-timelines -- /absolute/clip-directory`, then set
...
versions and exact cue positions. The [presentation contract](../docs/specs/arena-presentation-timelines.md)
